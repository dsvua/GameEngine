...
#include "../Renderer/CPUMesh.hpp"
#include "../Renderer/Color.hpp"
#include "../Renderer/GPUMesh.hpp"
#include "../Renderer/GfxDevice.hpp"
#include "../Renderer/MaterialCache.hpp"
#include "../Renderer/MeshCache.hpp"
...
static const std::string GLTF_SPECULAR_GLOSSINESS_KHR_KEY{"KHR_materials_pbrSpecularGlossiness"};
static const std::string GLTF_DIFFUSE_FACTOR_PARAM_NAME{"diffuseFactor"};
static const std::string GLTF_SPECULAR_FACTOR_PARAM_NAME{"specularFactor"};
static const std::string GLTF_GLOSSINESS_FACTOR_PARAM_NAME{"glossinessFactor"};
...
glm::vec4 getDiffuseColor(const tinygltf::Material& material)
{
    ...
    assert(c.size() == 4);
    return glm::vec4{(float)c[0], (float)c[1], (float)c[2], (float)c[3]};
}
...
glm::vec3 getEmissiveFactor(const tinygltf::Material& material)
{
    assert(material.emissiveFactor.size() == 3);
    return glm::vec3{(float)material.emissiveFactor[0], (float)material.emissiveFactor[1], (float)material.emissiveFactor[2]};
}

glm::vec4 getSpecularFactor(const tinygltf::Material& material)
{
    if (material.extensions.contains(GLTF_SPECULAR_GLOSSINESS_KHR_KEY))
    {
        const tinygltf::Value c = material.extensions.at(GLTF_EMISSIVE_STRENGTH_KHR_KEY).Get(GLTF_SPECULAR_FACTOR_PARAM_NAME);
        // assert(c.size() == 4);
        return glm::vec4{(float)c.Get(0).GetNumberAsDouble(), 
                (float)c.Get(1).GetNumberAsDouble(), 
                (float)c.Get(2).GetNumberAsDouble(), 
                (float)material.extensions.at(GLTF_EMISSIVE_STRENGTH_KHR_KEY).Get(GLTF_GLOSSINESS_FACTOR_PARAM_NAME).GetNumberAsDouble()};
    } else {
        return glm::vec4(1.0f, 1.0f, 1.0f, 1.0f - (float)material.pbrMetallicRoughness.roughnessFactor);
    }
}
...
{
    Material material{
        .diffuseFactor = getDiffuseColor(gltfMaterial),
        .specularFactor = getSpecularFactor(gltfMaterial),
    ...
    if (hasEmissiveTexture(gltfMaterial)) {
        material.emissiveStrength = getEmissiveStrength(gltfMaterial);
