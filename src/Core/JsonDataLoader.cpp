...
JsonDataLoader::JsonDataLoader(const nlohmann::json& data) : data(data), overrideMode(false)
{}

JsonDataLoader::JsonDataLoader(const nlohmann::json& data, std::string name) :
...
JsonDataLoader JsonDataLoader::getLoader(const std::string& key) const
{
    ...
    if (it != data.end()) {
        JsonDataLoader l(it.value(), std::format("{}.{}", name, key));
        l.setOverride(overrideMode);
        return l;
    }
    throw std::runtime_error(std::format("{} doesn't have a key {}", name, key));
}

JsonDataLoader JsonDataLoader::getLoader(std::size_t i) const
{
    if (i < data.size()) {
        JsonDataLoader l(data[i], std::format("{}[{}]", name, i));
        l.setOverride(overrideMode);
        return l;
    }
    throw std::runtime_error(std::format("Can't get {}-th value from {}: out of bounds", i, name));
}
...
std::unordered_map<std::string, JsonDataLoader> JsonDataLoader::getKeyValueMap() const
{
    ...
    for (const auto& [key, value] : data.items()) {
        JsonDataLoader loader(value, std::format("{}.{}", name, key));
...
std::vector<JsonDataLoader> JsonDataLoader::getVector() const
{
    ...
    int i = 0;
    for (auto& value : data) {
        JsonDataLoader loader(value, std::format("{}[{}]", name, i));
        loader.setOverride(overrideMode);
